...
def generar_excel(tipo, datos, archivo):
    ...
    titulo = "Socis Actius" if tipo == "actius" else "Socis Inactius"
    ws.title = titulo
...
def obtener_datos(tipo):
    ...
    query = (
        "SELECT * FROM socis WHERE Activitats IS NOT NULL AND Activitats != ''"
        if tipo == "actius"
        else "SELECT * FROM socis WHERE Activitats IS NULL OR Activitats = ''"
    )
...
root = tk.Tk()
root.title("Generar Llistes de Socis")
